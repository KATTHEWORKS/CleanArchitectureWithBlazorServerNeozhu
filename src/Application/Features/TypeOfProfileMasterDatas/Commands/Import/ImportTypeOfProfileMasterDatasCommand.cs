                 ...
                 IRequestHandler<ImportTypeOfProfileMasterDatasCommand, Result<int>>
    {
        ...
        public async Task<Result<int>> Handle(ImportTypeOfProfileMasterDatasCommand request, CancellationToken cancellationToken)
        {
           ...
           var result = await _excelService.ImportAsync(request.Data, mappers: new Dictionary<string, Func<DataRow, TypeOfProfileMasterDataDto, object?>>
            {
                { _localizer[_dto.GetMemberDescription(x=>x.SystemTypeId)], (row, item) => item.SystemTypeId = int.TryParse(row[_localizer[_dto.GetMemberDescription(x=>x.SystemTypeId)]].ToString() , out int resId)?resId: 0 },
...
{ _localizer[_dto.GetMemberDescription(x=>x.Price)], (row, item) => item.Price = int.TryParse( row[_localizer[_dto.GetMemberDescription(x=>x.Price)]].ToString(), out int resId) ? resId : 0 },  

...
public class AddEditTownProfileCommand: ICacheInvalidatorRequest<Result<int>>
{
      [Description("Id")]
    ...
    private class Mapping : Profile
    {
        public Mapping()
        {
            CreateMap<TownProfileDto,AddEditTownProfileCommand>(MemberList.None);
            CreateMap<AddEditTownProfileCommand,TownProfile>(MemberList.None);
         
        }
    }
}

    public class AddEditTownProfileCommandHandler : IRequestHandler<AddEditTownProfileCommand, Result<int>>
    {
        private readonly IApplicationDbContext _context;
        private readonly IMapper _mapper;
        private readonly IStringLocalizer<AddEditTownProfileCommandHandler> _localizer;
        public AddEditTownProfileCommandHandler(
            IApplicationDbContext context,
            IStringLocalizer<AddEditTownProfileCommandHandler> localizer,
            IMapper mapper
            )
        {
            _context = context;
        ...
        public async Task<Result<int>> Handle(AddEditTownProfileCommand request, CancellationToken cancellationToken)
        {
            if (request.Id > 0)
            {

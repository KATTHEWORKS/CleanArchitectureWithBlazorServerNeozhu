                 ...
                 IRequestHandler<ImportTownProfilesCommand, Result<int>>
    {
        ...
        public async Task<Result<int>> Handle(ImportTownProfilesCommand request, CancellationToken cancellationToken)
        {
           ...
           var result = await _excelService.ImportAsync(request.Data, mappers: new Dictionary<string, Func<DataRow, TownProfileDto, object?>>
            {
                { _localizer[_dto.GetMemberDescription(x=>x.TypeId)], (row, item) => item.TypeId = int.Parse( row[_localizer[_dto.GetMemberDescription(x=>x.TypeId)]].ToString()) }, 
...
{ _localizer[_dto.GetMemberDescription(x=>x.EndDateToShow)], (row, item) => item.EndDateToShow = DateTime.Parse(row[_localizer[_dto.GetMemberDescription(x=>x.EndDateToShow)]].ToString()) }, 
{ _localizer[_dto.GetMemberDescription(x=>x.PriotiryOrder)], (row, item) => item.PriotiryOrder = int.Parse(row[_localizer[_dto.GetMemberDescription(x=>x.PriotiryOrder)]].ToString()) }, 
...
{ _localizer[_dto.GetMemberDescription(x=>x.TownId)], (row, item) => item.TownId = int.Parse(row[_localizer[_dto.GetMemberDescription(x=>x.TownId)]].ToString()) }, 
{ _localizer[_dto.GetMemberDescription(x=>x.ApprovedCount)], (row, item) => item.ApprovedCount = int.Parse(row[_localizer[_dto.GetMemberDescription(x=>x.ApprovedCount)]].ToString()) }, 
{ _localizer[_dto.GetMemberDescription(x=>x.RejectedCount)], (row, item) => item.RejectedCount = int.Parse(row[_localizer[_dto.GetMemberDescription(x=>x.RejectedCount)]].ToString() )}, 
{ _localizer[_dto.GetMemberDescription(x=>x.LikeCount)], (row, item) => item.LikeCount = int.Parse(row[_localizer[_dto.GetMemberDescription(x=>x.LikeCount)]].ToString() )}, 
{ _localizer[_dto.GetMemberDescription(x=>x.DisLikeCount)], (row, item) => item.DisLikeCount = int.Parse(row[_localizer[_dto.GetMemberDescription(x=>x.DisLikeCount)]].ToString() )}, 
        ...
        public async Task<Result<byte[]>> Handle(CreateTownProfilesTemplateCommand request, CancellationToken cancellationToken)
        {
            ...
            var fields = new string[] {
                   ...
                   _localizer[_dto.GetMemberDescription(x=>x.TypeId)], 

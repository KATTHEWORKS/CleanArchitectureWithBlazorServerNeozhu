...
public class TownProfileAdvancedSpecification : Specification<TownProfile>
{
    public TownProfileAdvancedSpecification(TownProfileAdvancedFilter filter)
    {
       ...
       Query.Where(q => q.Name != null)
             ...
             .Where(q => q.CreatedBy == filter.CurrentUser.UserId, filter.ListView == TownProfileListView.My && filter.CurrentUser is not null)
             .Where(q => q.Created >= start && q.Created <= end, filter.ListView == TownProfileListView.CreatedToday)
             .Where(q => q.Created >= last30day, filter.ListView == TownProfileListView.Created30Days);

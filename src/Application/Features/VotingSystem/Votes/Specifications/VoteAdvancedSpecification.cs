...
public class VoteAdvancedSpecification : Specification<Vote>
{
    public VoteAdvancedSpecification(VoteAdvancedFilter filter)
    {
       ...
       Query
            //.Where(q => q.Name != null)
             ...
             .Where(q => q.CreatedBy == filter.CurrentUser.UserId, filter.ListView == VoteListView.My && filter.CurrentUser is not null)
             .Where(q => q.Created >= start && q.Created <= end, filter.ListView == VoteListView.CreatedToday)
             .Where(q => q.Created >= last30day, filter.ListView == VoteListView.Created30Days);

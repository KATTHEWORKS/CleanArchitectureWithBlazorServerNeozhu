...
public class TownAdvancedSpecification : Specification<Town>
{
    public TownAdvancedSpecification(TownAdvancedFilter filter)
    {
       ...
       Query.Where(q => q.Name != null)
             ...
             .Where(q => q.CreatedBy == filter.CurrentUser.UserId, filter.ListView == TownListView.My && filter.CurrentUser is not null)
             .Where(q => q.Created >= start && q.Created <= end, filter.ListView == TownListView.CreatedToday)
             .Where(q => q.Created >= last30day, filter.ListView == TownListView.Created30Days);

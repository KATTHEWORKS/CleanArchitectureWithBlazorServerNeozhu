...
public static class Perms
{
    ...
    //public static List<r> CommonReadRoles = AllRoles.ToList();
#if HOSPITAL_SYSTEM
    public static List<r> CommonReadRoles = AllRoles.Where(e => e >= r.PHARMACIST && e <= r.HOSPITAL).ToList();
#elif VOTING_SYSTEM
    public static List<r> CommonReadRoles = [r.Default];
#else
    public static List<r> CommonReadRoles = [r.Default];
#endif
    ...
    public static List<string> DefaultPermissions = AddPermissions(r.Default, CommonReadPermissions, p.CreateRestricted, p.UpdateRestricted, p.ReadRestricted);
    public static List<string> ElevateAdminViewerPermissions = AddPermissions(r.ELEVATEADMINVIEWER, AllReadPermissions)
        .Concat(BasePermissionsAll).Distinct().ToList();
#if HOSPITAL_SYSTEM
    ...
    ,(r.Default.ToString(),DefaultPermissions!)
    ...
    public static List<string> ElevateAdminGroupPermissions = AddPermissions(r.ELEVATEADMINGROUP, ElevateAdminViewerPermissions, CreateRoles)
       ...
       .Distinct().ToList();
    public static List<string> RootAdminPermissions = AddPermissions(r.ROOTADMIN, ElevateAdminGroupPermissions, CreateRoles)
        .Concat(RP(r.ELEVATEADMINGROUP, SubRolesUpdates))
        .Distinct().ToList();

    public static List<(string roleOrType, List<string> permissions)> PermissionsAll = new()
    {
        ("BasePermissionsAll", Permissions.GetRegisteredPermissions()),
        ("AllReadPermissions",RP(AllRoles.ToList(), p.Read)),
        ("CommonReadPermissions", RP(CommonReadRoles, p.Read)),
    (r.Default.ToString(),DefaultPermissions!),
        (r.ELEVATEADMINVIEWER.ToString(),ElevateAdminViewerPermissions!),
        (r.ELEVATEADMINGROUP.ToString(), ElevateAdminGroupPermissions!),
        (r.ROOTADMIN.ToString(), RootAdminPermissions!)
    };
#else
    public static List<string> ElevateAdminGroupPermissions = AddPermissions(r.ELEVATEADMINGROUP, ElevateAdminViewerPermissions, CreateRoles)
...
/* 
  //public static RolePermissions DefaultPermissions = new(r.PATIENT, AddPermissions(r.PATIENT, CommonReadPermissions, p.CreateRestricted, p.UpdateRestricted, p.ReadRestricted));
